...
def test_empty_states_use_lucide_icons(page: Page, live_server_url: str):
    ...
    if empty_state.is_visible():
        ...
        assert lucide_icon.count() > 0, "Empty state should use Lucide icon, not emoji"
...
def test_form_inputs_have_focus_states(page: Page, live_server_url: str):
    ...
    has_focus_indicator = (
        "rgb(29, 185, 84)" in box_shadow  # Green glow
        or outline != "none"
    )
...
def test_typography_uses_dm_sans(page: Page, live_server_url: str):
    ...
    assert "DM Sans" in font_family, f"Body should use DM Sans font, got: {font_family}"
